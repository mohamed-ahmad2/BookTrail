...
class _HomeScreenState extends State<HomeScreen> {
  final List<Book> books = [
    Book(title: "Book 1", author: "Author A", status: "read"),
    Book(title: "Book 2", author: "Author B", status: "want to read"),
    Book(title: "Book 3", author: "Author C", status: "reading"),
    Book(title: "Book 4", author: "Author D", status: "read"),
  ];
  ...
  List<Book> _filterBooks() {
    if (widget.tabController.index == 0) {
      ...
      return books.where((book) => book.status == "read").toList();
    } else if (widget.tabController.index == 2) {
      return books.where((book) => book.status == "want to read").toList();
